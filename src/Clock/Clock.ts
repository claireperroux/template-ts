...
export class Clock {
  ...
  private static readonly rowCapacity = 3; // Nombre maximal d'horloges par ligne
  private static readonly verticalMargin = 350; // Marge verticale entre les horloges
  private initialHourOffset: number;


  //xPosition: string, yPosition: string,
  constructor( hourOffset: number = 0) {
      ...
      //this.container.style.left = xPosition;
      //this.container.style.top = yPosition;
  ...
  private positionClock() {
    ...
    const initialTopOffset = 100; // Espace additionnel sous le bouton
    const rowNumber = Math.floor(Clock.count / Clock.rowCapacity);
    const colNumber = Clock.count % Clock.rowCapacity;
    this.container.style.left = `${colNumber * 33.33}%`;
    this.container.style.top = `${titleHeight + buttonHeight + initialTopOffset + rowNumber * (Clock.verticalMargin)}px`; // Calcul du top pour placer en dessous du titre et du bouton avec marges supplémentaires
    Clock.count++;
    }
